	...
	public sealed class TChannel: AChannel
	{
		...
		private void OnRecvComplete(object o)
		{
            ...
            while (true)
            {
                ...
                {
                    //从这里拿到数据流
                    MemoryStream ms = this.parser.GetPacket();
                    //this.OnRead(this.parser.GetPacket());
                    byte[] bb = ms.ToArray();
                    string json = System.Text.Encoding.UTF8.GetString(bb);
                    Log.Debug($"接收长度:{bb.Length},数据内容转UTF8:{json}");
                    Game.EventSystem.Run(MessageEventType.TCPOnReceive, json);
                }
                catch (Exception ee)
                {
                    Log.Error(ee);
